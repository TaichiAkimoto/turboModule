...
function typeToRNRawType(typeNode, sourceFile, options) {
    ...
    var itemStringLiterals = [];
    var itemNumberLiterals = [];
    var itemOthers = [];
    var scannedItems = [typeNode];
    for (var i = 0; i < scannedItems.length; i++) {
        var recognized = true;
        var item = scannedItems[i];
        if (ts.isTypeReferenceNode(item)) {
            var typeReferenceName = item.typeName.getText();
            switch (typeReferenceName) {
                ...
                case 'BubblingEventHandler': {
                    ...
                    if (item.typeArguments.length === 2) {
                        var nameNode = item.typeArguments[1];
            ...
            for (var _i = 0, _a = item.types; _i < _a.length; _i++) {
                var unionItem = _a[_i];
    ...
    if (itemOthers.length === 0) {
        if (itemNullable) {
            ...
            var scannedItemsInText = scannedItems.map(function (t) { return t.getText(); });
        ...
        var result = itemOthers.length === 1 ? itemOthers[0] : {
            kind: 'Union',
...
exports.typeToRNRawType = typeToRNRawType;
